...
def _download_onedrive(url: str, dest: str) -> None:
    """
    Download a file from a OneDrive / SharePoint sharing URL.

    OneDrive sharing links (1drv.ms, *-my.sharepoint.com, onedrive.live.com)
    return an HTML preview page by default.  Appending ``&download=1``
    (or ``?download=1`` when no query string exists) instructs the server to
    serve the raw binary instead.  curl -L already follows the redirect chain;
    the only missing piece was this query parameter.

    The function also validates the Content-Type returned by the server and
    writes the file only when it looks like a ZIP/OOXML archive.
    """
    import urllib.request
    import urllib.error

    # ── 1. Inject download=1 unless already present ──────────────────────────
    if 'download=1' not in url:
        sep = '&' if '?' in url else '?'
        url = f"{url}{sep}download=1"

    headers = {
        # Some CDN nodes reject non-browser User-Agents and return HTML instead.
        'User-Agent': (
    ...
    try:
        ...
        raise RuntimeError(
            f"HTTP {exc.code} while downloading the Excel file: {exc.reason}\n"
            f"URL: {url}"
        ) from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(
            f"Network error while downloading the Excel file: {exc.reason}\n"
            f"URL: {url}"
        ) from exc

    _assert_xlsx_bytes(raw, url, content_type)

    with open(dest, 'wb') as fh:
        fh.write(raw)
    print(f"  Saved {len(raw):,} bytes → {dest}")


def _assert_xlsx_bytes(data: bytes, source_hint: str = '', content_type: str = '') -> None:
    """
    Raise a *descriptive* RuntimeError if *data* is not a valid XLSX archive.

    Checks (in order):
      1. Non-empty
      2. ZIP magic bytes  PK\\x03\\x04
      3. Valid ZIP structure (no truncation / corruption)
      4. Office Open XML marker  [Content_Types].xml  present inside the ZIP

    Each failure path prints an actionable fix message aimed at the most likely
    CI root cause (OneDrive returning HTML instead of the binary file).
    """
    import zipfile
    import io
    ...
    if not data:
        raise RuntimeError(
            ...
            "Likely cause: the OneDrive secret URL is stale or the file was moved.\n"
            f"Source: {source_hint}"
        )

    # ── Guard 2: ZIP magic bytes ──────────────────────────────────────────────
    if not data.startswith(b'PK\x03\x04'):
        snippet = data[:512]
        lower   = snippet.lower()

        if b'<html' in lower or b'<!doctype' in lower:
            preview = snippet.decode('utf-8', errors='replace')[:300]
            raise RuntimeError(
                "OneDrive returned an HTML page instead of the Excel binary.\n\n"
                "ROOT CAUSE\n"
                "----------\n"
                "OneDrive / SharePoint sharing URLs serve an HTML preview by default.\n"
                "The workflow's curl command fetched that page, saved it with a .xlsx\n"
                "extension, and openpyxl failed when it tried to treat HTML as a ZIP.\n\n"
                "FIX (choose one)\n"
                "----------------\n"
                "A) The updated workflow (update-equipment-3.yml) already appends\n"
                "   '&download=1' automatically — re-run with the new YAML.\n"
                "B) In GitHub Secrets → ONEDRIVE_EXCEL_URL, replace the sharing\n"
                "   URL with a direct-download link:\n"
                "     • SharePoint / OneDrive for Business:\n"
                "         original: https://iitmacin-my.sharepoint.com/:x:/g/…?e=xxx\n"
                "         fixed:    https://iitmacin-my.sharepoint.com/:x:/g/…?e=xxx&download=1\n"
                "     • Personal OneDrive (1drv.ms short link):\n"
                "         fixed:    https://1drv.ms/x/s!<token>?download=1\n\n"
                f"HTML PREVIEW RECEIVED (first 300 chars):\n{preview}\n\n"
                f"Content-Type header: {content_type or '(not available)'}\n"
                f"Source: {source_hint}"
            )

        raise RuntimeError(
            "Downloaded file does not begin with ZIP magic bytes (PK\\x03\\x04).\n"
            f"First 16 bytes (hex): {data[:16].hex()}\n"
            f"File size received  : {len(data):,} bytes\n"
            f"Content-Type header : {content_type or '(not available)'}\n"
            f"Source              : {source_hint}"
        )

    # ── Guard 3 + 4: valid ZIP and OOXML marker ───────────────────────────────
    try:
        with zipfile.ZipFile(io.BytesIO(data), 'r') as zf:
            names = zf.namelist()
    except zipfile.BadZipFile as exc:
        raise RuntimeError(
            f"File starts with PK bytes but is a corrupt / truncated ZIP: {exc}\n"
            f"File size: {len(data):,} bytes  |  Source: {source_hint}"
        ) from exc

    if '[Content_Types].xml' not in names:
        raise RuntimeError(
            "File is a valid ZIP archive but is NOT an Office Open XML document\n"
            "(missing '[Content_Types].xml' entry).\n"
            f"ZIP entries found: {names[:15]}\n"
            f"Source: {source_hint}"
        )


def validate_xlsx(filepath: str) -> None:
    """
    On-disk variant of *_assert_xlsx_bytes* — used when the file was
    downloaded externally (e.g. by curl in CI) and we just need to
    validate what is already on disk before handing it to openpyxl.
    """
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Excel file not found: {filepath}")

    file_size = os.path.getsize(filepath)
    print(f"  Validating: {filepath}  ({file_size:,} bytes)")

    with open(filepath, 'rb') as fh:
        data = fh.read()

    _assert_xlsx_bytes(data, source_hint=filepath)
    print("  Validation passed — file is a well-formed XLSX archive.")


# ── End download + validation helpers ─────────────────────────────────────────
...
if __name__ == '__main__':
    ...
    validate_xlsx(source)          # Fail fast with an actionable error if curl
    equipment = extract_data(source)  # fetched HTML instead of the real xlsx.
